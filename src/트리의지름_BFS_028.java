...
public class 트리의지름_BFS_028 {
    public static void main(String[] args) {
        ...
        ArrayList<Edge>[] A = new ArrayList[N + 1]; // 노드마다 인접리스트 구현
        ...
        BFS(max, visited, distance, A); // 가장 긴 index로 BFS 다시

        Arrays.sort(distance); // 오름차순 정렬
        System.out.println(distance[N]); // 가장 끝 index => 가장 긴 지름 출력
    }

    private static void BFS(int v, boolean[] visited, int[] distance, ArrayList<Edge>[] A) {
