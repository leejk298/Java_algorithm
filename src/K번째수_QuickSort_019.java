...
public class K번째수_QuickSort_019 {
    ...
    private static void quickSort(int[] A, int S, int E, int K) { // 퀵 정렬
        if (S < E) {
    ...
    private static int partition(int[] A, int S, int E) { // 분할
        if (S + 1 == E) { // 두 개인 경우
            if (A[S] > A[E]) { // 비교 후 스왑
                swap(A, S, E);
            }
    ...
    private static void swap(int[] A, int S, int E) { // 스왑
        int tmp = A[S];
        A[S] = A[E];
        A[E] = tmp;
    }
